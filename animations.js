// Corteyo Animations — scroll observer + nav effects + counters + whatsapp
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const statSelectors = '[class*="text-4xl"], [class*="text-5xl"], [class*="text-6xl"], [class*="text-7xl"]';
  ...
  statEls.forEach(el => {
    if (parseStatEl(el)) counterObserver.observe(el);
  });

  // ── 4. Make email & phone clickable ──
  function linkifyText(node) {
    if (node.nodeType === 3) { // text node
      ...
      let html = text
        ...
        .replace(emailRe, '<a href="mailto:$1" style="color:#23C8C8;text-decoration:underline;font-weight:600;">$1</a>')
        .replace(phoneRe, '<a href="tel:$1" style="color:#23C8C8;text-decoration:underline;font-weight:600;">$1</a>');
      span.innerHTML = html;
      node.parentNode.replaceChild(span, node);
    } else if (
      node.nodeType === 1 &&
      !['A','SCRIPT','STYLE','NOSCRIPT'].includes(node.tagName)
    ) {
      Array.from(node.childNodes).forEach(linkifyText);
    }
  }
  // Run after React renders (small delay)
  setTimeout(() => linkifyText(document.body), 800);
  ...
  // Show tooltip after 3 seconds then auto-hide
  setTimeout(() => {
